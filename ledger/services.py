...
def export_month_region_metadata_excel(record_month: str, region: str) -> bytes:
    """Export a batch-entry Excel with metadata for all projects under one region and month."""
    if region not in VALID_REGIONS:
        raise ValueError(f"Unsupported region: {region}")

    projects = [project for project in list_projects() if project["region"] == region]
    batch_id = f"{region}-{record_month}"
    ...
    for project in projects:
        columns = list_project_columns(project["id"])
        row: dict[str, Any] = {
            "batch_id": batch_id,
        ...
        for column in columns:
            if column.name not in seen_column_names:
                seen_column_names.add(column.name)
                union_columns.append(column.name)
            row[column.name] = ""
    ...
    data_columns = ["batch_id", "project_id", "project_name", "region", "record_month", "notes", *union_columns]
    data_df = pd.DataFrame(data_rows)
    if data_df.empty:
        data_df = pd.DataFrame(columns=data_columns)
    else:
        data_df = data_df.reindex(columns=data_columns, fill_value="")
        ...
        [
            {
                ...
                "instructions": "Fill values in data_entry sheet and import this file for batch monthly update.",
            }
        ]
    )
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        data_df.to_excel(writer, sheet_name="data_entry", index=False)
        dictionary_df.to_excel(writer, sheet_name="column_dictionary", index=False)
    return output.getvalue()


def _normalize_record_month(value: Any) -> str:
    if value is None:
        return ""
...
def import_month_region_metadata_excel(file_bytes: bytes) -> dict[str, int]:
    """Import the batch-entry Excel generated by export_month_region_metadata_excel."""
    workbook = pd.ExcelFile(BytesIO(file_bytes))
    if "data_entry" not in workbook.sheet_names:
        raise ValueError("Invalid template: missing data_entry sheet.")

    dataframe = workbook.parse("data_entry")
    required = {"project_id", "record_month"}
    missing = [column for column in required if column not in dataframe.columns]
    if missing:
        raise ValueError(f"Invalid template: missing required columns: {', '.join(missing)}")
    ...
    for row in dataframe.to_dict(orient="records"):
        ...
        month = _normalize_record_month(row.get("record_month"))
        if pd.isna(project_id_raw) or not month:
            skipped_rows += 1
            continue
        ...
        columns = list_project_columns(project_id)
        name_to_id = {column.name: column.id for column in columns}
        values = {column_id: row.get(column_name, "") for column_name, column_id in name_to_id.items()}
        notes_raw = row.get("notes", "")
        notes = "" if pd.isna(notes_raw) else str(notes_raw).strip()
        save_monthly_record(project_id, month, notes, values)
        imported_records += 1
